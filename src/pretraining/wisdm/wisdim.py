...
DATA_DIR = r"C:\codes\HumanActivityRecognition\data\WISDM"
OUTPUT_DIR = os.path.join(os.path.dirname(DATA_DIR), "WISDM_100Hz")

BEST_MODEL_PATH = os.path.join(MODELS_DIR, f"best_deepconvlstm_wisdm_seed_{SEED}.pt")
BEST_SCORE_PATH = os.path.join(MODELS_DIR, f"best_deepconvlstm_wisdm_seed_{SEED}.score.txt")

best_hyperparams_file = os.path.join(REPORTS_DIR, f'best_hyperparameters_wisdm_seed_{SEED}.csv')

FIGURES_DIR = r"C:\codes\HumanActivityRecognition\reports\figures"
...
def plot_wisdm_subject_activity_distribution(DATA_DIR, FIGURES_DIR):
    """
    ...
    ann_files = [f for f in os.listdir(DATA_DIR) if f.endswith("_ann.npz")]
    ...
    for sig_file in sig_files:
        ...
        try:
            sig = np.load(os.path.join(DATA_DIR, sig_file), allow_pickle=True)
            ann = np.load(os.path.join(DATA_DIR, ann_file), allow_pickle=True)
...
def main():
    ...
    #Creazione dataset
    train_dataset = HARDataset(X_train_cf, Y_train_enc, class_names=SELECTED_ACTIVITIES)
    ...
    figure_name=f"cm_train_best_wisdm_seed_{SEED}"
)
    ...
    plot_CM(
        ...
        Y=Y_test_enc,
        batch_size=best_batch_size,
        figure_name=f"cm_test_best_wisdm_seed_{SEED}"
    )
