...
best_hyperparams_file = os.path.join(REPORTS_DIR, f'best_hyperparameters_wisdm_medianiqr_sampler_seed_{SEED}.csv')
...
def compute_median_iqr_from_windows(X: np.ndarray):
    """
    Calcola mediane e IQR per canale su tutte le finestre (N,L,C).
    """
    reshaped = X.reshape(-1, X.shape[2])   # (N*L, C)
    medians = np.median(reshaped, axis=0)
    iqrs = np.percentile(reshaped, 75, axis=0) - np.percentile(reshaped, 25, axis=0)
    iqrs[iqrs == 0] = 1e-6  # evita divisione per zero
    return medians, iqrs

def normalize_with_median_iqr(X: np.ndarray, medians: np.ndarray, iqrs: np.ndarray):
    """
...
def main():
    ...
    medians, iqrs = compute_median_iqr_from_windows(X_train)
    print("Medians:", medians)
    print("IQRs:", iqrs)

    # === Normalizzazione train e test con quelle statistiche ===
    X_train = normalize_with_median_iqr(X_train, medians, iqrs)
    X_test  = normalize_with_median_iqr(X_test,  medians, iqrs)
    ...
    train_sampler = create_weighted_sampler(Y_train_enc)
    ...
    figure_name=f"cm_train_best_wisdm_medianiqr_sampler_seed_{SEED}"
)
    ...
    plot_CM(
        ...
        Y=Y_test_enc,
        batch_size=best_batch_size,
        figure_name=f"cm_test_best_wisdm_medianiqr_sampler_seed_{SEED}"
    )
