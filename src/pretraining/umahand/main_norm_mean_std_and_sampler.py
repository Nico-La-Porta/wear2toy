...
def main():
    ...
    best_hyperparams_file = os.path.join(REPORTS_DIR, 'best_hyperparameters_dl_norm_mean_std_and_sampler.csv')


    # Definisco i percorsi per i modelli e i migliori risultati
    # Definisco i percorsi per i modelli e i migliori risultati
    BEST_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_dl_norm_mean_std_and_sampler.pkl")
    BEST_SCORE_PATH = os.path.join(REPORTS_DIR, "best_score_dl_norm_mean_std_and_sampler.txt")
    ...
    if os.path.exists(best_hyperparams_file):
        ...
        net = DeepConvLSTM()
        train_with_cm.train(net, train_loader, test_loader, epochs=100, batch_size=best_batch_size, lr=best_lr)
        # Il modello verrà salvato come da tua funzione train (se serve, aggiungi qui torch.save)
        torch.save(net.state_dict(), BEST_MODEL_PATH)
        logger.info(f"Modello salvato in: {BEST_MODEL_PATH}")
    else:
        ...
        study = optuna.create_study(
            study_name="dl_hyperparam_search",
        ...
        best_hyperparameters_df.to_csv(os.path.join(REPORTS_DIR, 'best_hyperparameters_dl_norm_mean_std_and_sampler.csv'), index=False)
        ...
        file_name = "optimization_history_dl_norm_mean_std_and_sampler.png"
    ...
    plot_CM(
        ...
        figure_name="cm_train_best_hyp_optuna_dl_norm_mean_std_and_sampler"
    )
    ...
    plot_CM(
        ...
        Y=Y_Test,
        batch_size=best_batch_size,
        figure_name="cm_test_best_hyp_optuna_dl_norm_mean_std_and_sampler"
    )
    ...
    test_loss, test_acc, test_f1 = train_with_cm.evaluate_model(model, complete_dataloader, figure_name= "cm_final_eval_best_hyp_optuna_dl_norm_mean_std_and_sampler", save_confusion_matrix=True)
