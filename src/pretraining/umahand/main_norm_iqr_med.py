...
BEST_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_dl_norm_med_iqr_without_sampler.pkl")
BEST_SCORE_PATH = os.path.join(REPORTS_DIR, "best_score_dl_norm_med_iqr_without_sampler.txt")
...
global_medians, global_iqr = normalization.compute_global_median_iqr(dataset_train_labled_conc) #calcolo mediana e IQR sul training set

#salvo global medians e iqr per usarli in futuro in formato csv 

global_medians_df = pd.DataFrame(global_medians, columns=["median"])
global_iqr_df = pd.DataFrame(global_iqr, columns=["iqr"])
global_medians_df.to_csv(os.path.join(REPORTS_DIR, 'global_medians_trs.csv'), index=False)
global_iqr_df.to_csv(os.path.join(REPORTS_DIR, 'global_iqr_trs.csv'), index=False)


scaled_train_data = normalization.normalize_each_recording_med_iqr(dataset_train_labled, global_medians, global_iqr) #TODO: sostituisci questa funzione facendo la normalizzazione direttamente nella collate function quando carichi i dati
logger.info("Train data scalers applied")

del dataset_train_labled

scaled_test_data = normalization.normalize_each_recording_med_iqr(dataset_test_labled, global_medians, global_iqr)
...
best_hyperparams_file = os.path.join(REPORTS_DIR, 'best_hyperparameters_dl_norm_med_iqr_without_sampler.csv')
if os.path.exists(best_hyperparams_file):
    ...
    best_hyperparameters_df.to_csv(os.path.join(REPORTS_DIR, 'best_hyperparameters_dl_norm_med_iqr_without_sampler.csv'), index=False)
    ...
    file_name = "optimization_history_dl_norm_med_iqr_without_sampler.png"
...
plot_CM(
    ...
    figure_name="cm_train_best_hyp_optuna_dl_norm_med_iqr_without_sampler"
)
...
plot_CM(
    ...
    Y=Y_Test,
    batch_size=best_batch_size,
    figure_name="cm_test_best_hyp_optuna_dl_norm_med_iqr_without_sampler"
)
...
test_loss, test_acc, test_f1 = train_with_cm.evaluate_model(model, complete_dataloader, figure_name= "cm_final_eval_best_hyp_optuna_dl_norm_med_iqr_without_sampler", save_confusion_matrix=True)
