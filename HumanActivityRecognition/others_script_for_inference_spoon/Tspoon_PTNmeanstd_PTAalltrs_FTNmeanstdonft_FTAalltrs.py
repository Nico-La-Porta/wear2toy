...
means, stds = normalization.compute_dataframe_mean_std(df_spoon_filtered)

logger.info(f"Media delle colonne: {means}")
logger.info(f"Deviazione standard delle colonne: {stds}")


df_spoon_normalized = normalization.normalize_dataframe_mean_std(df_spoon_filtered, means, stds)


#ora divido il dataframe in base all'attività (action_id) e salvo i dataframe in un file .csv
#per ogni attività

for action_id in df_spoon_normalized['action_id'].unique():
    df_action = df_spoon_normalized[df_spoon_filtered["action_id"] == action_id] #filtro il dataframe in base all'attività
    ...
    #salvo il dataframe
    df_action.to_csv(os.path.join(path,f'df_normalized_ft_spoon_action_{action_id}.csv'),index=False) #index=False per non salvare l'indice
    logger.debug(f"Salvato il dataframe df_normalized_ft_spoon_action_{action_id}.csv")
...
for action_file in [f for f in os.listdir(path) if f.endswith('.csv') and f.split('_')[1] == 'normalized' and f.split('_')[2]=='ft' and f.split('_')[3] == 'spoon']:
    ...
    kid_action_counts[f"spoon_action_{action_id}"] = kid_id_action_dict #aggiungo il dizionario al dizionario principale per tenere traccia del numero di finestre per ogni bambino per ogni azione, ogni spoon_action è una chiave e il valore è un dizionario con il numero di finestre per ogni bambino
...
for fold, (train_val_idx, test_idx) in enumerate(skf.split(X, Y)):
    ...
    X_trs_aug = transformation_function(X_train_fold)
    Y_trs_aug = Y_train_fold.copy()

    # Concateno originale e augmentato
    X_trs_final = np.concatenate([X_train_fold, X_trs_aug], axis=0)
    Y_trs_final = np.concatenate([Y_train_fold, Y_trs_aug], axis=0)
    ...
    test_loss, test_acc, test_f1_fold = train_with_cm.evaluate_model(
        ...
        figure_name=f"cm_test_fold_{fold + 1}_kfold_inference_3_folds_Tspoon_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs",
    ...
    model_path_fold = os.path.join(BEST_MODEL_KFOLD_DIR, f"best_model_fold_{fold + 1}_Tspoon_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs.pkl")
...
results_df.to_csv(os.path.join(figures_spoon_path, 'kfold_results_inference_spoon_Tspoon_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs.csv'), index=False)
...
best_hyperparameters_df.to_csv(
    os.path.join(figures_spoon_path, 'best_hyperparameters_kfold_inference_spoon_Tspoon_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs.csv'), 
    index=False
)
...
FINAL_MODEL_PATH = os.path.join(BEST_MODEL_KFOLD_DIR, "best_model_kfold_final_inference_spoon_Tspoon_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs.pkl")
...
plot_CM(
    ...
    figure_name="cm_final_kfold_inference_spoon_all_data_Tspoon_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs",
    figures_dir=figures_spoon_path,
    reports_dir=figures_spoon_path,
)
...
cm_combined, y_true, y_pred = combine_kfold_confusion_matrices(
    ...
    num_folds=3,
    toy_name="spoon",
    save_path="combined_cm_Tspoon_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs.png",
