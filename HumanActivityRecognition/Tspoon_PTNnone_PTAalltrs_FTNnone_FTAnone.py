...
for fold, (train_val_idx, test_idx) in enumerate(skf.split(X, Y)):
    ...
    test_loss, test_acc, test_f1_fold = train_with_cm.evaluate_model(
        ...
        figure_name=f"cm_test_fold_{fold + 1}_kfold_inference_3_folds_Tspoon_PTNnone_PTAalltrs_FTNnone_FTAnone",
    ...
    model_path_fold = os.path.join(BEST_MODEL_KFOLD_DIR, f"best_model_fold_{fold + 1}_Tspoon_PTNnone_PTAalltrs_FTNnone_FTAnone.pkl")
...
results_df.to_csv(os.path.join(REPORTS_DIR, 'kfold_results_inference_spoon_Tspoon_PTNnone_PTAalltrs_FTNnone_FTAnone.csv'), index=False)
...
best_hyperparameters_df.to_csv(
    os.path.join(REPORTS_DIR, 'best_hyperparameters_kfold_inference_spoon_Tspoon_PTNnone_PTAalltrs_FTNnone_FTAnone.csv'), 
    index=False
)
