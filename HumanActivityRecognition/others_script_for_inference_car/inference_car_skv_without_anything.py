...
OPTUNA_RESULTS_PATH = os.path.join(REPORTS_DIR, 'kfold_results_inference_car.csv')
OPTUNA_HYPERPARAMS_PATH = os.path.join(REPORTS_DIR, 'best_hyperparameters_kfold_inference_car.csv')
OPTUNA_ALREADY_RUN = os.path.exists(OPTUNA_RESULTS_PATH) and os.path.exists(OPTUNA_HYPERPARAMS_PATH)
...
if OPTUNA_ALREADY_RUN:
    ...
    for fold, (train_val_idx, test_idx) in enumerate(skf.split(X, Y)):
        ...
        model_final_fold.fc = nn.Linear(num_ftrs, 10)
        model_final_fold.set_n_classes(10)
        ...
        test_loss, test_acc, test_f1_fold = train_with_cm.evaluate_model(
            ...
            figure_name=f"cm_test_fold_{fold + 1}_kfold_inference_car",
...
# Calcolo statistiche sui fold
    ...
    results_df.to_csv(os.path.join(REPORTS_DIR, 'kfold_results_inference_car.csv'), index=False)
    ...
    best_hyperparameters_df.to_csv(
        os.path.join(REPORTS_DIR, 'best_hyperparameters_kfold_inference_car.csv'), 
        index=False
    )
...
FINAL_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_kfold_final_inference_car.pkl")
...
plot_CM(
    ...
    figure_name="cm_final_kfold_inference_car_all_data"
)
...
final_model.fc = nn.Linear(num_ftrs, 10)
...
test_loss, test_acc, test_f1 = train_with_cm.evaluate_model(
    ...
    figure_name=f"cm_holdout_kid_{KID_TEST}_final_eval",
    save_confusion_matrix=True,
    save_f1_score=True
)
