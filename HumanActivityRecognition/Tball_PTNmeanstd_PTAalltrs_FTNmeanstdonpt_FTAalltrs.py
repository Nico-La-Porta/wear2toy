...
BEST_MODEL_KFOLD_DIR = os.path.join(MODELS_DIR, "kfold_ball_models_3_folds_Tball_PTNmeanstd_PTAalltrs_FTNmeanstdonpt_FTAalltrs")
...
for fold, (train_val_idx, test_idx) in enumerate(skf.split(X, Y_mapped)):
    ...
    test_loss, test_acc, test_f1_fold = train_with_cm.evaluate_model(
        ...
        figure_name=f"cm_test_fold_{fold + 1}_kfold_Tball_PTNmeanstd_PTAalltrs_FTNmeanstdonpt_FTAalltrs",
...
results_df.to_csv(os.path.join(REPORTS_DIR, 'kfold_results_inference_ball_3_folds_Tball_PTNmeanstd_PTAalltrs_FTNmeanstdonpt_FTAalltrs.csv'), index=False)
...
best_hyperparameters_df.to_csv(
    os.path.join(REPORTS_DIR, 'best_hyperparameters_kfold_inference_ball_3_folds_Tball_PTNmeanstd_PTAalltrs_FTNmeanstdonpt_FTAalltrs.csv'), 
    index=False
)
...
FINAL_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_kfold_final_inference_ball_3_folds_Tball_PTNmeanstd_PTAalltrs_FTNmeanstdonpt_FTAalltrs.pkl")
...
plot_CM(
    ...
    figure_name="cm_final_kfold_inference_ball_all_data_3_folds_Tball_PTNmeanstd_PTAalltrs_FTNmeanstdonpt_FTAalltrs",
    labels_dict=labels_dict
)
...
cm_combined, y_true, y_pred = combine_kfold_confusion_matrices(
    ...
    num_folds=3,
    toy_name="ball",
    save_path="combined_cm_ball_3fold_Tball_PTNmeanstd_PTAalltrs_FTNmeanstdonpt_FTAalltrs.png",
