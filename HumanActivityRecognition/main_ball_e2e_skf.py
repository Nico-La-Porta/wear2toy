...
from figures import combine_kfold_confusion_matrices, plot_CM
...
for fold, (train_val_idx, test_idx) in enumerate(skf.split(X, Y_mapped)):
    ...
    test_loss, test_acc, test_f1_fold = train_with_cm.evaluate_model(
        ...
        figure_name=f"cm_test_fold_{fold + 1}_kfold_e2e_ball_3_folds",
...
results_df.to_csv(os.path.join(REPORTS_DIR, 'kfold_results_e2e_ball_3_folds_without_anything.csv'), index=False)
...
best_hyperparameters_df.to_csv(
    os.path.join(REPORTS_DIR, 'best_hyperparameters_kfold_e2e_ball_3_folds_without_anything.csv'), 
    index=False
)
...
FINAL_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_kfold_final_e2e_ball_3_folds_without_anything.pkl")
...
plot_CM(
    ...
    figure_name="cm_final_kfold_e2e_ball_all_data_3_folds_without_anything",
    labels_dict=labels_dict
)
