...
for action_id in df_spoon_filtered['action_id'].unique():
    ...
    df_action = pd.read_csv(os.path.join(path, f'df_spoon_action_{action_id}.csv'))
    ...
    normalized_file_path = os.path.join(path, f'df_spoon_action_{action_id}_normalized_pt.csv')
...
X, Y = [], []
kid_action_counts = {}
normalized_files = [f for f in os.listdir(path) if f.endswith('_normalized_pt.csv') and 'spoon_action' in f]
...
for action_file in normalized_files:
    ...
    kid_action_counts[f"spoon_action_{action_id}"] = kid_id_action_dict #aggiungo il dizionario al dizionario principale per tenere traccia del numero di finestre per ogni bambino per ogni azione, ogni ball_action è una chiave e il valore è un dizionario con il numero di finestre per ogni bambino
...
for fold, (train_val_idx, test_idx) in enumerate(skf.split(X, Y)):
    ...
    test_loss, test_acc, test_f1_fold = train_with_cm.evaluate_model(
        ...
        figure_name=f"cm_test_fold_{fold + 1}_kfold_inference_3_folds_Tspoon_PTNmeanstd_PTAalltrs_FTNmeanstdonpt_FTAalltrs",
    ...
    model_path_fold = os.path.join(BEST_MODEL_KFOLD_DIR, f"best_model_fold_{fold + 1}_Tspoon_PTNmeanstd_PTAalltrs_FTNmeanstdonpt_FTAalltrs.pkl")
...
results_df.to_csv(os.path.join(figures_spoon_path, 'kfold_results_inference_spoon_Tspoon_PTNmeanstd_PTAalltrs_FTNmeanstdonpt_FTAalltrs.csv'), index=False)
...
best_hyperparameters_df.to_csv(
    os.path.join(figures_spoon_path, 'best_hyperparameters_kfold_inference_spoon_Tspoon_PTNmeanstd_PTAalltrs_FTNmeanstdonpt_FTAalltrs.csv'), 
    index=False
)
...
FINAL_MODEL_PATH = os.path.join(BEST_MODEL_KFOLD_DIR, "best_model_kfold_final_inference_spoon_Tspoon_PTNmeanstd_PTAalltrs_FTNmeanstdonpt_FTAalltrs.pkl")
...
plot_CM(
    ...
    figure_name="cm_final_kfold_inference_spoon_all_data_Tspoon_PTNmeanstd_PTAalltrs_FTNmeanstdonpt_FTAalltrs",
    figures_dir=figures_spoon_path,
    reports_dir=figures_spoon_path,
)
