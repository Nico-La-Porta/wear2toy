...
loaded_data = np.load(f"{path}\\split_final_indices_car.npz")
...
BEST_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_inference_car_norm_mean_std_and_aug_timeseries.pkl")
BEST_SCORE_PATH = os.path.join(REPORTS_DIR, "best_score_inference_car_norm_mean_std_and_aug_timeseries.txt")

best_hyperparams_file = os.path.join(REPORTS_DIR, 'best_hyperparameters_inference_inference_car_norm_mean_std_and_aug_timeseries.csv')
if os.path.exists(best_hyperparams_file):
    ...
    def objective(trial):
        ...
        model.load_state_dict(torch.load(r'C:\codes\HumanActivityRecognition\models\best_model_dl_norm_mean_std_and_aug.pkl', map_location=torch.device('cpu')), strict=False)
    ...
    best_hyperparameters_df.to_csv(os.path.join(REPORTS_DIR, 'best_hyperparameters_inference_inference_car_norm_mean_std_and_aug_timeseries.csv'), index=False)
    ...
    file_name = "optimization_history_inference_car_norm_mean_std_and_aug_timeseries.png"
...
plot_CM(
    ...
    figure_name="cm_train_best_hyp_inference_optuna_inference_car_norm_mean_std_and_aug_timeseries"
)
...
plot_CM(
    ...
    Y=Y_val_mapped,
    batch_size=best_batch_size,
    figure_name="cm_val_best_hyp_inference_optuna_inference_car_norm_mean_std_and_aug_timeseries"
)

# Creo modello e carico pesi del miglior modello (trovato prima in optuna)
model = DeepConvLSTM(n_classes=29)
...
test_loss, test_acc, test_f1 = train_with_cm.evaluate_model(model, test_loader, figure_name= "cm_test_best_hyp_optuna_inference_car_norm_mean_std_and_aug_timeseries", save_confusion_matrix=True)
