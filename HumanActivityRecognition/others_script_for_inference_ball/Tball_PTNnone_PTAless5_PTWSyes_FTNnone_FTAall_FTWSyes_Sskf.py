...
from models.DeepConvLSTM import DeepConvLSTM, HARDataset , create_weighted_sampler, collate_fn
import optuna
...
tranform_1 = transformation_function(X)
X.shape, tranform_1.shape


print(f"Dimensioni di X_Train: {X.shape}")
print(f"Dimensioni di Y_Train: {Y.shape}")
X_augmented = np.concatenate((X, tranform_1), axis=0)
Y_augmented = np.concatenate((Y, Y), axis=0)
print(f"Dimensioni di X_Train_augmented: {X_augmented.shape}")
print(f"Dimensioni di Y_Train_augmented: {Y_augmented.shape}")



# Stampo le dimensioni di X e Y
logger.info(f"Dimensioni di X finale: {X_augmented.shape}")
logger.info(f"Dimensioni di Y finale: {Y_augmented.shape}")

logger.debug(f"Tipo di Y: {type(Y_augmented)}")
logger.debug(f"Tipo del primo elemento di Y: {type(Y_augmented[0])}")
logger.debug(f"Shape del primo elemento di Y: {Y_augmented[0].shape if hasattr(Y[0], 'shape') else 'No shape'}")
logger.debug(f"Primi 5 elementi di Y: {Y_augmented[:5]}")


# Trova tutte le etichette uniche e crea mapping
unique_labels = np.unique(Y_augmented)
label_mapping = {label: idx for idx, label in enumerate(unique_labels)}
logger.info("Mapping delle etichette: %s", label_mapping)
Y_flat = Y_augmented.flatten()  # Converte da (28, 1) a (28,)
# Applica il mapping
...
for fold, (train_val_idx, test_idx) in enumerate(skf.split(X_augmented, Y_mapped)):
    logger.info(f"\n=== FOLD {fold + 1}/{K_FOLDS} ===")
    
    # Split dei dati per questo fold
    X_train_val, X_test_fold = X_augmented[train_val_idx], X_augmented[test_idx]
    ...
    def objective_fold(trial):
        ...
        # Creo i DataLoader
        train_loader = DataLoader(train_dataset_fold, batch_size=batch_size, drop_last=True, sampler=train_sampler, collate_fn=collate_fn)
        ...
        model.load_state_dict(
            torch.load(r'C:\codes\HumanActivityRecognition\models\best_model_dl_with_aug_only_5_classes_and_sampler.pkl', 
        ...
        # Congelo tutti i parametri tranne la testa
    ...
    num_ftrs = model_final_fold.fc.in_features
    model_final_fold.fc = nn.Linear(num_ftrs, 4)
    model_final_fold.set_n_classes(4)
    
    for param in model_final_fold.parameters():
        param.requires_grad = False
    for param in model_final_fold.fc.parameters():
        param.requires_grad = True

    # Training finale per questo fold
    train_loader_final = DataLoader(train_dataset_fold, batch_size=best_batch_size, drop_last=True, sampler=train_sampler, collate_fn=collate_fn)
    ...
    test_loss, test_acc, test_f1_fold = train_with_cm.evaluate_model(
        ...
        figure_name=f"cm_test_fold_{fold + 1}_kfold_inference_ball_3_folds_Tball_PTNnone_PTAless5_PTWSyes_FTNnone_FTAall_FTWSyes_Sskf",
    ...
    model_path_fold = os.path.join(BEST_MODEL_KFOLD_DIR, f"best_model_fold_{fold + 1}_Tball_PTNnone_PTAless5_PTWSyes_FTNnone_FTAall_FTWSyes_Sskf.pkl")
...
results_df.to_csv(os.path.join(REPORTS_DIR, 'kfold_results_inference_ball_3_folds_Tball_PTNnone_PTAless5_PTWSyes_FTNnone_FTAall_FTWSyes_Sskf.csv'), index=False)
...
best_hyperparameters_df.to_csv(
    os.path.join(REPORTS_DIR, 'best_hyperparameters_kfold_inference_ball_3_folds_Tball_PTNnone_PTAless5_PTWSyes_FTNnone_FTAall_FTWSyes_Sskf.csv'), 
    index=False
)
...
full_dataset = HARDataset(X_augmented, Y_mapped)
sampler= create_weighted_sampler(Y_mapped)
...
train_loader_final = DataLoader(train_dataset_final, batch_size=best_batch_size_final,drop_last=True, sampler=sampler, collate_fn=collate_fn)
...
model_final.fc = nn.Linear(num_ftrs, 4)
model_final.set_n_classes(4)

for param in model_final.parameters():
    param.requires_grad = False
for param in model_final.fc.parameters():
    param.requires_grad = True
...
FINAL_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_kfold_final_inference_ball_3_folds_Tball_PTNnone_PTAless5_PTWSyes_FTNnone_FTAall_FTWSyes_Sskf.pkl")
...
plot_CM(
    ...
    figure_name="cm_final_kfold_inference_ball_all_data_3_folds_Tball_PTNnone_PTAless5_PTWSyes_FTNnone_FTAall_FTWSyes_Sskf",
    labels_dict=labels_dict
)
...
cm_combined, y_true, y_pred = combine_kfold_confusion_matrices(
    ...
    num_folds=3,
    toy_name="ball",
    save_path="combined_cm_ball_3fold_Tball_PTNnone_PTAless5_PTWSyes_FTNnone_FTAall_FTWSyes_Sskf",
