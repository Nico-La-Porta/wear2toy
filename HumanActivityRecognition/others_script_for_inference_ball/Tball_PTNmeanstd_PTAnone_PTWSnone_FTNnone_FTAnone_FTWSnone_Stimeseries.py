...
create_single_distribution_bar_chart(
    Y=Y,
    toy_name="BALL",
    title_suffix="Complete Dataset",
    save_path=os.path.join(FIGURES_DIR, "ball_complete_distribution")
)
...
BEST_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_inference_ball_Tball_PTNmeanstd_PTAnone_PTWSnone_FTNnone_FTAnone_FTWSnone_Stimeseries.pkl")
BEST_SCORE_PATH = os.path.join(REPORTS_DIR, "best_score_inference_ball_Tball_PTNmeanstd_PTAnone_PTWSnone_FTNnone_FTAnone_FTWSnone_Stimeseries.txt")

best_hyperparams_file = os.path.join(REPORTS_DIR, 'best_hyperparameters_inference_inference_ball_Tball_PTNmeanstd_PTAnone_PTWSnone_FTNnone_FTAnone_FTWSnone_Stimeseries.csv')
if os.path.exists(best_hyperparams_file):
    ...
    def objective(trial):
        ...
        model.load_state_dict(torch.load(r'C:\codes\HumanActivityRecognition\models\best_model_dl_norm_mean_std_without_sampler.pkl', map_location=torch.device('cpu')), strict=False)
    ...
    best_hyperparameters_df.to_csv(os.path.join(REPORTS_DIR, 'best_hyperparameters_inference_inference_ball_Tball_PTNmeanstd_PTAnone_PTWSnone_FTNnone_FTAnone_FTWSnone_Stimeseries.csv'), index=False)
...
plot_CM(
    ...
    figure_name="cm_train_best_hyp_inference_optuna_inference_ball_Tball_PTNmeanstd_PTAnone_PTWSnone_FTNnone_FTAnone_FTWSnone_Stimeseries",
...
plot_CM(
    ...
    Y=Y_val_mapped,
    batch_size=best_batch_size,
    figure_name="cm_val_best_hyp_inference_optuna_inference_ball_Tball_PTNmeanstd_PTAnone_PTWSnone_FTNnone_FTAnone_FTWSnone_Stimeseries",
    labels_dict=labels_dict
)
...
test_loss, test_acc, test_f1 = train_with_cm.evaluate_model(model, test_loader, figure_name= "cm_test_best_hyp_optuna_inference_ball_Tball_PTNmeanstd_PTAnone_PTWSnone_FTNnone_FTAnone_FTWSnone_Stimeseries", save_confusion_matrix=True, save_f1_score= True,  labels_dict=labels_dict)
