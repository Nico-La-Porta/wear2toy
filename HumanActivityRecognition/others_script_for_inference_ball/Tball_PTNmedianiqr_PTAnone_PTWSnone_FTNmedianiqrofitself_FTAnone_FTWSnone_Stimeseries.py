...
medians, iqr = normalization.compute_dataframe_median_iqr(df_ball)

logger.info(f"Mediana delle colonne: {medians}")
logger.info(f"Iqr delle colonne: {iqr}")


df_ball_normalized = normalization.normalize_dataframe_median_iqr(df_ball, medians, iqr)

#ora divido il dataframe in base all'attività (action_id) e salvo i dataframe in un file .csv
#per ogni attività

for action_id in df_ball_normalized['action_id'].unique():
    df_action = df_ball_normalized[df_ball["action_id"] == action_id] #filtro il dataframe in base all'attività
    ...
    #salvo il dataframe
    df_action.to_csv(os.path.join(path,f'df_normalizedmediqritself_ball_action_{action_id}.csv'),index=False) #index=False per non salvare l'indice
    logger.debug(f"Salvato il dataframe df_normalizedmediqritself_ball_action_{action_id}.csv")
...
for action_file in [f for f in os.listdir(path) if f.endswith('.csv') and f.split('_')[1] == 'normalizedmediqritself' and f.split('_')[2]=='ball']:
...
BEST_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_inference_Tball_PTNmedianiqr_PTAnone_PTWSnone_FTNmedianiqrofitself_FTAnone_FTWSnone_Stimeseries.pkl")
BEST_SCORE_PATH = os.path.join(REPORTS_DIR, "best_score_inference_Tball_PTNmedianiqr_PTAnone_PTWSnone_FTNmedianiqrofitself_FTAnone_FTWSnone_Stimeseries.txt")

best_hyperparams_file = os.path.join(REPORTS_DIR, 'best_hyperparameters_inference_inference_Tball_PTNmedianiqr_PTAnone_PTWSnone_FTNmedianiqrofitself_FTAnone_FTWSnone_Stimeseries.csv')
if os.path.exists(best_hyperparams_file):
    ...
    def objective(trial):
        ...
        model.load_state_dict(torch.load(r'C:\codes\HumanActivityRecognition\models\best_model_dl_norm_med_iqr_without_sampler.pkl', map_location=torch.device('cpu')), strict=False)
    ...
    best_hyperparameters_df.to_csv(os.path.join(REPORTS_DIR, 'best_hyperparameters_inference_inference_Tball_PTNmedianiqr_PTAnone_PTWSnone_FTNmedianiqrofitself_FTAnone_FTWSnone_Stimeseries.csv'), index=False)
    ...
    file_name = "optimization_history_inference_ball_without_anythinh.png"
...
plot_CM(
    ...
    figure_name="cm_train_best_hyp_inference_optuna_inference_Tball_PTNmedianiqr_PTAnone_PTWSnone_FTNmedianiqrofitself_FTAnone_FTWSnone_Stimeseries",
...
plot_CM(
    ...
    Y=Y_val_mapped,
    batch_size=best_batch_size,
    figure_name="cm_val_best_hyp_inference_optuna_inference_Tball_PTNmedianiqr_PTAnone_PTWSnone_FTNmedianiqrofitself_FTAnone_FTWSnone_Stimeseries",
    labels_dict=labels_dict
)
...
test_loss, test_acc, test_f1 = train_with_cm.evaluate_model(model, test_loader, figure_name= "cm_test_best_hyp_optuna_inference_Tball_PTNmedianiqr_PTAnone_PTWSnone_FTNmedianiqrofitself_FTAnone_FTWSnone_Stimeseries", save_confusion_matrix=True, save_f1_score= True,  labels_dict=labels_dict)
