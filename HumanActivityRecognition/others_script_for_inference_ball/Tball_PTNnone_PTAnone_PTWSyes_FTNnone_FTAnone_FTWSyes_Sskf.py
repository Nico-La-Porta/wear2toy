...
for fold, (train_val_idx, test_idx) in enumerate(skf.split(X, Y_mapped)):
    ...
    def objective_fold(trial):
        ...
        model.load_state_dict(
            torch.load(r'C:\codes\HumanActivityRecognition\models\best_model_dl_with_sampler.pkl', 
    ...
    test_loss, test_acc, test_f1_fold = train_with_cm.evaluate_model(
        ...
        figure_name=f"cm_test_fold_{fold + 1}_kfold_inference_ball_3_folds",
    ...
    model_path_fold = os.path.join(BEST_MODEL_KFOLD_DIR, f"best_model_fold_{fold + 1}_Tball_PTNnone_PTAnone_PTAWSyes_FTNnone_FTAnone_FTWSyes_Sskf.pkl")
...
results_df.to_csv(os.path.join(REPORTS_DIR, 'kfold_results_inference_ball_3_folds_Tball_PTNnone_PTAnone_PTAWSyes_FTNnone_FTAnone_FTWSyes_Sskf.csv'), index=False)
...
best_hyperparameters_df.to_csv(
    os.path.join(REPORTS_DIR, 'best_hyperparameters_kfold_inference_ball_3_folds_Tball_PTNnone_PTAnone_PTAWSyes_FTNnone_FTAnone_FTWSyes_Sskf.csv'), 
    index=False
)
...
train_loader_final = DataLoader(train_dataset_final, batch_size=best_batch_size_final, drop_last=True, shuffle=True, collate_fn=collate_fn)
...
FINAL_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_kfold_final_inference_ball_3_folds_Tball_PTNnone_PTAnone_PTAWSyes_FTNnone_FTAnone_FTWSyes_Sskf.pkl")
...
plot_CM(
    ...
    figure_name="cm_final_kfold_inference_ball_all_data_3_folds_Tball_PTNnone_PTAnone_PTAWSyes_FTNnone_FTAnone_FTWSyes_Sskf",
    labels_dict=labels_dict
)
...
cm_combined, y_true, y_pred = combine_kfold_confusion_matrices(
    ...
    num_folds=3,
    toy_name="ball",
    save_path="combined_cm_ball_3fold_Tball_PTNnone_PTAnone_PTAWSyes_FTNnone_FTAnone_FTWSyes_Sskf.png",
