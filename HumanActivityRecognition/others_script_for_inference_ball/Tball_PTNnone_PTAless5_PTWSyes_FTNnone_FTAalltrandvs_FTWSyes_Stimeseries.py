...
trasform_2= transformation_function(X_val)
X_train.shape, tranform_1.shape
X_val.shape, trasform_2.shape
...
Y_Train_augmented = np.concatenate((Y_train_mapped, Y_train_mapped), axis=0)
...
X_Val_augmented = np.concatenate((X_val, trasform_2), axis=0)
Y_Val_augmented = np.concatenate((Y_val_mapped, Y_val_mapped), axis=0)
print(f"Dimensioni di X_Val_augmented: {X_Val_augmented.shape}")
print(f"Dimensioni di Y_Val_augmented: {Y_Val_augmented.shape}")
...
val_dataset = HARDataset(X_Val_augmented, Y_Val_augmented)
...
BEST_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_inference_Tball_PTNnone_PTAless5_PTWSyes_FTNnone_FTAalltrandvs_FTWSyes_Stimeseries.pkl")
BEST_SCORE_PATH = os.path.join(REPORTS_DIR, "best_score_inference_Tball_PTNnone_PTAless5_PTWSyes_FTNnone_FTAalltrandvs_FTWSyes_Stimeseries.txt")

best_hyperparams_file = os.path.join(REPORTS_DIR, 'best_hyperparameters_inference_inference_Tball_PTNnone_PTAless5_PTWSyes_FTNnone_FTAalltrandvs_FTWSyes_Stimeseries.csv')
if os.path.exists(best_hyperparams_file):
    ...
    def objective(trial):
        ...
        model.load_state_dict(torch.load(r'C:\codes\HumanActivityRecognition\models\best_model_dl_with_aug_only_5_classes_and_sampler.pkl', map_location=torch.device('cpu')), strict=False)
    ...
    best_hyperparameters_df.to_csv(os.path.join(REPORTS_DIR, 'best_hyperparameters_inference_inference_Tball_PTNnone_PTAless5_PTWSyes_FTNnone_FTAalltrandvs_FTWSyes_Stimeseries.csv'), index=False)
...
plot_CM(
    ...
    figure_name="cm_train_best_hyp_inference_optuna_inference_Tball_PTNnone_PTAless5_PTWSyes_FTNnone_FTAalltrandvs_FTWSyes_Stimeseries",
...
plot_CM(
    ...
    Y=Y_Val_augmented,
    batch_size=best_batch_size,
    figure_name="cm_val_best_hyp_inference_optuna_inference_Tball_PTNnone_PTAless5_PTWSyes_FTNnone_FTAalltrandvs_FTWSyes_Stimeseries",
    labels_dict=labels_dict,
)
...
test_loss, test_acc, test_f1 = train_with_cm.evaluate_model(model, test_loader, figure_name= "cm_test_best_hyp_optuna_inference_Tball_PTNnone_PTAless5_PTWSyes_FTNnone_FTAalltrandvs_FTWSyes_Stimeseries", save_confusion_matrix=True, save_f1_score= True, class_names=class_names)
