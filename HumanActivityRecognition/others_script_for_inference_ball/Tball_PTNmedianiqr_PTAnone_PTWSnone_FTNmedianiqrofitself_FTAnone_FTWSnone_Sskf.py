...
for fold, (train_val_idx, test_idx) in enumerate(skf.split(X, Y_mapped)):
    ...
    def objective_fold(trial):
        ...
        model.load_state_dict(
            torch.load(r'C:\codes\HumanActivityRecognition\models\best_model_dl_norm_med_iqr_without_sampler.pkl', 
    ...
    test_loss, test_acc, test_f1_fold = train_with_cm.evaluate_model(
        ...
        figure_name=f"cm_test_fold_{fold + 1}_kfold" + this_script_name,
...
results_df.to_csv(os.path.join(REPORTS_DIR, 'kfold_results_inference_ball_3_folds_Tball_PTNmedianiqr_PTAnone_PTWSnone_FTNmedianiqrofpt_FTAnone_FTWSnone_Sskf.csv'), index=False)
...
best_hyperparameters_df.to_csv(
    os.path.join(REPORTS_DIR, 'best_hyperparameters_kfold_inference_ball_3_folds_Tball_PTNmedianiqr_PTAnone_PTWSnone_FTNmedianiqrofpt_FTAnone_FTWSnone_Sskf.csv'), 
    index=False
)
...
FINAL_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_kfold_final_inference_ball_3_folds_Tball_PTNmedianiqr_PTAnone_PTWSnone_FTNmedianiqrofpt_FTAnone_FTWSnone_Sskf.pkl")
...
plot_CM(
    ...
    figure_name="cm_final_kfold" + this_script_name,
    labels_dict=labels_dict
)
...
cm_combined, y_true, y_pred = combine_kfold_confusion_matrices(
    ...
    num_folds=3,
    toy_name="ball",
    save_path="combined_cm_ball_3fold" + this_script_name + ".png",
