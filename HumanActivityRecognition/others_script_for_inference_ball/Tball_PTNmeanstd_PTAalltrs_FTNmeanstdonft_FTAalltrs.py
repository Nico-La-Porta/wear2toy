...
for action_id in df_ball_normalized['action_id'].unique():
    ...
    #salvo il dataframe
    df_action.to_csv(os.path.join(path,f'df_normalized_ft_ball_action_{action_id}.csv'),index=False) #index=False per non salvare l'indice
    logger.debug(f"Salvato il dataframe df_normalized_ft_ball_action_{action_id}.csv")
...
for action_file in [f for f in os.listdir(path) if f.endswith('.csv') and f.split('_')[1] == 'normalized' and f.split('_')[2]=='ft' and f.split('_')[3] == 'ball']:
...
BEST_MODEL_KFOLD_DIR = os.path.join(MODELS_DIR, "kfold_ball_models_3_folds_Tball_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs")
...
for fold, (train_val_idx, test_idx) in enumerate(skf.split(X, Y_mapped)):
    ...
    test_loss, test_acc, test_f1_fold = train_with_cm.evaluate_model(
        ...
        figure_name=f"cm_test_fold_{fold + 1}_kfold_Tball_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs",
...
results_df.to_csv(os.path.join(REPORTS_DIR, 'kfold_results_inference_ball_3_folds_Tball_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs.csv'), index=False)
...
best_hyperparameters_df.to_csv(
    os.path.join(REPORTS_DIR, 'best_hyperparameters_kfold_inference_ball_3_folds_Tball_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs.csv'), 
    index=False
)
...
Y_trainval_final = np.concatenate([Y_mapped, Y_aug], axis=0)
...
FINAL_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_kfold_final_inference_ball_3_folds_Tball_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs.pkl")
...
plot_CM(
    ...
    figure_name="cm_final_kfold_inference_ball_all_data_3_folds_Tball_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs",
    labels_dict=labels_dict
)
...
cm_combined, y_true, y_pred = combine_kfold_confusion_matrices(
    ...
    num_folds=3,
    toy_name="ball",
    save_path="combined_cm_ball_3fold_Tball_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs.png",
