...
results_df.to_csv(os.path.join(REPORTS_DIR, 'kfold_results_inference_ball_3_folds_without_anything.csv'), index=False)
...
best_hyperparameters_df.to_csv(
    os.path.join(REPORTS_DIR, 'best_hyperparameters_kfold_inference_ball_3_folds_without_anything.csv'), 
    index=False
)
...
FINAL_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_kfold_final_inference_ball_3_folds_without_anything.pkl")
...
plot_CM(
    ...
    figure_name="cm_final_kfold_inference_ball_all_data_3_folds_without_anything",
    labels_dict=labels_dict
)
...
cm_combined, y_true, y_pred = combine_kfold_confusion_matrices(
    ...
    num_folds=3,
    toy_name="ball",
    save_path="combined_cm_ball_3fold_without_anything.png",
