...
# Carico median e std salvati durante il pre-training
logger.debug(f"REPORTS_DIR attuale: {REPORTS_DIR}")
iqr_df = pd.read_csv(os.path.join(REPORTS_DIR, 'global_iqr_trs.csv'))
median_df = pd.read_csv(os.path.join(REPORTS_DIR, 'global_medians_trs.csv'))

median = median_df['median'].values
iqr = iqr_df['iqr'].values
...
logger.info("Parametri di normalizzazione filtrati")
logger.debug(f"Median shape: {median.shape}, IQR shape: {iqr.shape}")
...
for action_id in df_ball['action_id'].unique():
    ...
    # Applico la normalizzazione usando mediana e IQR
    df_action_normalized = normalization.normalize_dataframe_median_iqr(df_action, median, iqr)
    
    # Salvo il DataFrame normalizzato
    normalized_file_path = os.path.join(path, f'df_ball_action_{action_id}_median_normalized_pt.csv')
...
X, Y = [], []
kid_action_counts = {}
normalized_files = [f for f in os.listdir(path) if f.endswith('_median_normalized_pt.csv') and 'ball_action' in f]
...
BEST_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_inference_Tball_PTNmedianiqr_PTAnone_PTWSnone_FTNmedianiqrofpt_FTAnone_FTWSnone_Stimeseries.pkl")
BEST_SCORE_PATH = os.path.join(REPORTS_DIR, "best_score_inference_Tball_PTNmedianiqr_PTAnone_PTWSnone_FTNmedianiqrofpt_FTAnone_FTWSnone_Stimeseries.txt")

best_hyperparams_file = os.path.join(REPORTS_DIR, 'best_hyperparameters_inference_inference_Tball_PTNmedianiqr_PTAnone_PTWSnone_FTNmedianiqrofpt_FTAnone_FTWSnone_Stimeseries.csv')
if os.path.exists(best_hyperparams_file):
    ...
    best_hyperparameters_df.to_csv(os.path.join(REPORTS_DIR, 'best_hyperparameters_inference_inference_Tball_PTNmedianiqr_PTAnone_PTWSnone_FTNmedianiqrofpt_FTAnone_FTWSnone_Stimeseries.csv'), index=False)
...
plot_CM(
    ...
    figure_name="cm_train_best_hyp_inference_optuna_inference_Tball_PTNmedianiqr_PTAnone_PTWSnone_FTNmedianiqrofpt_FTAnone_FTWSnone_Stimeseries",
...
plot_CM(
    ...
    Y=Y_val_mapped,
    batch_size=best_batch_size,
    figure_name="cm_val_best_hyp_inference_optuna_inference_Tball_PTNmedianiqr_PTAnone_PTWSnone_FTNmedianiqrofpt_FTAnone_FTWSnone_Stimeseries",
    labels_dict=labels_dict
)
...
test_loss, test_acc, test_f1 = train_with_cm.evaluate_model(model, test_loader, figure_name= "cm_test_best_hyp_optuna_inference_Tball_PTNmedianiqr_PTAnone_PTWSnone_FTNmedianiqrofpt_FTAnone_FTWSnone_Stimeseries", save_confusion_matrix=True, save_f1_score= True,  labels_dict=labels_dict)
