...
for action_file in [f for f in os.listdir(path_1) if f.endswith('.csv') and f.split('_')[1] == 'doll']:
...
OPTUNA_RESULTS_PATH = os.path.join(REPORTS_DIR, 'kfold_results_Tdoll_PTNnone_PTAalltrs_FTNnone_FTAalltrs.csv')
OPTUNA_HYPERPARAMS_PATH = os.path.join(REPORTS_DIR, 'best_hyperparameters_kfold_Tdoll_PTNnone_PTAalltrs_FTNnone_FTAalltrs.csv')
...
FINAL_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_kfold_final_Tdoll_PTNnone_PTAalltrs_FTNnone_FTAalltrs.pkl")
...
plot_CM(
    ...
    figure_name="cm_final_kfold_Tdoll_PTNnone_PTAalltrs_FTNnone_FTAalltrs_all_data_trainval",
...
test_loss, test_acc, test_f1 = train_with_cm.evaluate_model(
    ...
    figure_name=f"cm_holdout_kid_{KID_TEST}_final_eval_Tdoll_PTNnone_PTAalltrs_FTNnone_FTAalltrs",
