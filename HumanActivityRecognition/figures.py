...
def plot_CM(mdl_class, mdl_weights: str, X: np.ndarray, Y: np.ndarray, batch_size: int, figure_name: str, n_classes: int= 29, labels_dict: dict = None):
    """
    ...
    try:
        ...
        logger.info(f"Input shape: {X.shape}, Labels shape: {Y.shape}")
        model = mdl_class(n_classes=n_classes, nb_sensor_channels=X.shape[1], sliding_window_length=100)
        # Carico i pesi NEL modello, non sull'OrderedDict
        ...
        with torch.no_grad():
            for batch in dataloader:
                # GESTIONE FLESSIBILE DEL NUMERO DI VALORI RESTITUITI DAL DATALOADER
                if len(batch) == 2:
                    data, labels = batch
                elif len(batch) == 3:
                    data, labels, _ = batch  # Ignora il terzo elemento (metadata)
                elif len(batch) == 4:
                    data, labels, _, _ = batch  # Ignora terzo e quarto elemento
                else:
                    # Fallback: prendi i primi due elementi
