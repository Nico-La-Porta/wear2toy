...
BEST_MODEL_PATH = os.path.join(MODELS_DIR, f"best_deepconvlstm_wisdm_medianiqr_aug_seed_{SEED}.pt")
BEST_SCORE_PATH = os.path.join(MODELS_DIR, f"best_deepconvlstm_wisdm_medianiqr_aug_seed_{SEED}.score.txt")

best_hyperparams_file = os.path.join(REPORTS_DIR, f'best_hyperparameters_wisdm_medianiqr_aug_seed_{SEED}.csv')
...
def main():
    ...
    means, stds = compute_median_iqr_from_windows(X_train)
    print("Means:", means)
    print("Stds:", stds)

    # === Normalizzazione train e test con quelle statistiche ===
    X_train = normalize_with_median_iqr(X_train, means, stds)
    X_test  = normalize_with_median_iqr(X_test,  means, stds)
    ...
    figure_name=f"cm_train_best_wisdm_medianiqr_aug_seed_{SEED}"
)
    ...
    plot_CM(
        ...
        Y=Y_test_enc,
        batch_size=best_batch_size,
        figure_name=f"cm_test_best_wisdm_medianiqr_aug_seed_{SEED}"
    )
