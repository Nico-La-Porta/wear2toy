...
BEST_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_spoon_e2e.pkl")
BEST_SCORE_PATH = os.path.join(REPORTS_DIR, "best_score_spoon_e2e.txt")

best_hyperparams_file = os.path.join(REPORTS_DIR, 'best_hyperparameters_spoon_e2e.csv')
if os.path.exists(best_hyperparams_file):
    ...
    best_hyperparameters_df.to_csv(os.path.join(REPORTS_DIR, 'best_hyperparameters_spoon_e2e.csv'), index=False)
...
plot_CM(
    ...
    figure_name="cm_train_best_spoon_e2e"
)
...
plot_CM(
    ...
    Y=Y_val,
    batch_size=best_batch_size,
    figure_name="cm_val_best_hyp_spoon_e2e"
)
...
test_loss, test_acc, test_f1 = train_with_cm.evaluate_model(model, test_loader, figure_name= "cm_test_best_hyp_spoon_e2e",  save_confusion_matrix=True, save_f1_score= True)