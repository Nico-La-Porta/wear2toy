...
np.random.seed(SEED)
random.seed(SEED)
OUTPUT_WISDIM_DATA_DIR = os.path.join(DATA_DIR, 'WISDM_100Hz')
...
BEST_MODEL_PATH = os.path.join(MODELS_DIR, f"best_deepconvlstm_wisdm__aug_sampler_seed_{SEED}.pt")
BEST_SCORE_PATH = os.path.join(MODELS_DIR, f"best_deepconvlstm_wisdm__aug_sampler_seed_{SEED}.score.txt")

best_hyperparams_file = os.path.join(REPORTS_DIR, f'best_hyperparameters_wisdm__aug_sampler_seed_{SEED}.csv')
...
def sensor_fixed_permutations_transform(X):
    """
    Applica le 6 permutazioni fisse di orientamento ai canali
    Accelerometro (0-2) e Giroscopio (3-5).
    
    Parametri
    ---------
    X : np.ndarray
        Dati con shape (N, C, T), dove C >= 6
    
    Ritorna
    -------
    X_aug : np.ndarray
        Dati aumentati con tutte le 6 permutazioni concatenate.
    """
    N, C, T = X.shape
    
    # Permutazioni canoniche per assi (x,y,z)
    perms = [
        [0, 1, 2],  # originale
        [1, 2, 0],  # shift
        [2, 0, 1],  # altro shift
    ...
    augmented = []
    for p in perms:
        # ricostruisci i canali applicando la stessa permutazione ad Acc e Gyr
        idx = p + [i+3 for i in p]  # [acc_perm + gyr_perm]
        Xp = X[:, idx, :]
        augmented.append(Xp)
...
def main():
    ...
    print("After encoding/CF:")
    print("Train:", X_train_cf.shape, Y_train_enc.shape)
    print("Test:", X_test_cf.shape, Y_test_enc.shape)
    ...
    figure_name=f"cm_train_best_wisdm_aug_sampler_seed_{SEED}"
)
    ...
    plot_CM(
        ...
        Y=Y_test_enc,
        batch_size=best_batch_size,
        figure_name=f"cm_test_best_wisdm_aug_sampler_seed_{SEED}"
    )
