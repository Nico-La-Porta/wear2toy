...
train_loader = DataLoader(train_dataset, batch_size=12, drop_last=True,sampler=train_sampler, collate_fn=collate_fn)
test_loader = DataLoader(test_dataset, batch_size=12, shuffle=False, drop_last=True)
# Crea il modello con gli iperparametri suggeriti
net = DeepConvLSTM(n_classes=19)
# Esegui l'allenamento con i migliori iperparametri
# Carica i migliori iperparametri


# Estrai i valori
lr = 0.0027637706856868567
bs = 12

best_f1_score = train_toys_with_cm.train(net, train_loader, test_loader, epochs=100, batch_size= bs, lr=lr, cm_filename="Confusion Matrix BE")
