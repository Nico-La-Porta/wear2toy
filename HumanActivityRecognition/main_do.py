...
print("Dimensioni del df_DOcon tutte le attività non nulle")
print(df_be.shape)
print(df_be.columns)
print(df_be['action'].value_counts())

#salvo il dataframe
df_be.to_csv(os.path.join(path,'df_DO_non_null.csv'),index=False)
...
for action_id in df_be['action_id'].unique():
    ...
    #salvo il dataframe
    df_action.to_csv(os.path.join(path,f'df_do_action_{action_id}.csv'),index=False) #index=False per non salvare l'indice
    print(f"Salvato il dataframe df_do_action_{action_id}.csv")
...
for file in os.listdir(path):
    if file.endswith('.csv') and file.split('_')[1] == 'do':
...
print("Numero totale di finestre per il giocattolo do:")
print(X.shape)
print(Y.shape)
...
best_params_file = os.path.join(REPORTS_DIR, "best_params_do.json")
