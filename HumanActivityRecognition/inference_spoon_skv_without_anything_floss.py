...
# Calcola i pesi delle classi PRIMA del ciclo K-Fold
...
BEST_MODEL_KFOLD_DIR = os.path.join(MODELS_DIR, "kfold_spoon_models_3_folds_floss")
...
for fold, (train_val_idx, test_idx) in enumerate(skf.split(X, Y)):
    ...
    def objective_fold(trial):
        ...
        focal_criterion = FocalLoss(
            gamma=2.0,          # Focus sulle classi difficili
            alpha=class_weights,  # Usa i pesi già calcolati
            reduction='mean',
        ...
        best_f1_score = train_with_cm.train(model, train_loader, val_loader, 
                                           epochs=100, batch_size=batch_size, lr=lr, criterion=focal_criterion)
    ...
    model_final_fold.fc = nn.Linear(num_ftrs, 3)
    ...
    final_f1_score = train_with_cm.train(model_final_fold, train_loader_final, val_loader_final,
                                        epochs=100, batch_size=best_batch_size, lr=best_lr, criterion=focal_criterion)
    ...
    test_loss, test_acc, test_f1_fold = train_with_cm.evaluate_model(
        ...
        figure_name=f"cm_test_fold_{fold + 1}_kfold_inference_spoon_3_folds_floss",
...
results_df.to_csv(os.path.join(REPORTS_DIR, 'kfold_results_inference_spoon_3_folds_floss.csv'), index=False)
...
best_hyperparameters_df.to_csv(
    os.path.join(REPORTS_DIR, 'best_hyperparameters_kfold_inference_spoon_3_folds_floss.csv'), 
    index=False
)
...
model_final.fc = nn.Linear(num_ftrs, 3)
...
final_f1_score = train_with_cm.train(
    ...
    epochs=100, batch_size=best_batch_size_final, lr=best_lr_final, criterion= focal_criterion,
)

# Salvo il modello finale
FINAL_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_kfold_final_inference_spoon_3_folds_floss.pkl")
...
plot_CM(
    ...
    figure_name="cm_final_kfold_inference_spoon_all_data_3_folds_floss",
)
...
cm_combined, y_true, y_pred = combine_kfold_confusion_matrices(
    ...
    num_folds=3,
    toy_name="spoon",
    save_path="combined_cm_spoon_3fold_floss.png"
)
