...
#CREO SOTTOCARTELLA NELLA CARTELLA FIGURES_DIR CHE SI CHIAMA ball_figures_lp
figures_ball_path = os.path.join(FIGURES_DIR, 'visualization_error_ball')
os.makedirs(figures_ball_path, exist_ok=True)  # Crea la cartella
...
if os.path.exists(final_csv_path):
    ...
    logger.info(f"Shape finale del dataframe: {df_ball.shape}")
    logger.info(f"Colonne del dataframe: {list(df_ball.columns)}")
    logger.info(f"Conteggio delle azioni:\n{df_ball['action'].value_counts()}")
...
#logger.info("FILTRAGGIO CLASSI DAL DATASET BALL")

"""# Mostra distribuzione originale
logger.info("Distribuzione action_id originale:")
original_counts = df_ball['action_id'].value_counts().sort_index()
...
df_ball_filtered = df_ball[~df_ball['action_id'].isin(classes_to_remove)] # filtro il dataframe per rimuovere le classi che non mi interessano

logger.info(f"Righe prima del filtro: {len(df_ball)}")
logger.info(f"Righe dopo il filtro: {len(df_ball_filtered)}")
logger.info(f"Righe rimosse: {len(df_ball) - len(df_ball_filtered)}")


logger.info("Distribuzione action_id dopo filtro:")
filtered_counts = df_ball_filtered['action_id'].value_counts().sort_index()
...
for action_id in df_ball['action_id'].unique():
    ...
    #salvo il dataframe
    df_action.to_csv(os.path.join(figures_ball_path,f'df_ball_action_{action_id}.csv'),index=False) #index=False per non salvare l'indice
...
for action_file in [f for f in os.listdir(figures_ball_path) if f.endswith('.csv') and f.startswith('df_ball_action_') and not f.endswith('normalized_pt.csv')]:
...
class_names = [ball_mapping["encoded_to_name"][i] for i in range(3)]
...
#CREO UN MAPPING RIGA -> FINESTRA: Costruisco un dizionario che per ogni riga del dataframe df_ball annota 
...
for window_info in all_window_indices:
    ...
    file_name = f'df_ball_action_{action_id}.csv'
...
output_dir = os.path.join(figures_ball_path, "df_with_windows")
os.makedirs(output_dir, exist_ok=True)  # Crea la cartella se non esiste
for action_file in [f for f in os.listdir(path) if f.startswith("df_ball_action_") and f.endswith(".csv") and not f.endswith('normalized_pt.csv')]:
...
prediction_path="C:\codes\HumanActivityRecognition\reports\Tball_PTNnone_PTAnone_FTNnone_FTAnone_TSfft"
import ast
window_correct_map = {}
# Carico tutte le predizioni in un dizionario window_id → correct
prediction_files = [f for f in os.listdir(prediction_path) if f.startswith("predictions_TEST") and f.endswith(".csv")]
for prediction_file in prediction_files:
    prediction_df = pd.read_csv(os.path.join(figures_ball_path, prediction_file))
...
input_dir = os.path.join(figures_ball_path, "df_with_windows")
output_dir = os.path.join(figures_ball_path, "df_with_windows_with_correct")
os.makedirs(output_dir, exist_ok=True)

for file in os.listdir(input_dir):
    if not file.startswith("df_ball_action_") or not file.endswith(".csv"):
        continue
...
plot_sensor_channels_with_colored_line_and_windows(
    r'C:\codes\HumanActivityRecognition\data\downstream_data\df_with_windows_with_correct\df_ball_action_10.csv',
    signal_cols
)

plot_sensor_channels_with_colored_line_and_windows(
    r'C:\codes\HumanActivityRecognition\data\downstream_data\df_with_windows_with_correct\df_ball_action_11.csv',
    signal_cols
)

plot_sensor_channels_with_colored_line_and_windows(
    r'C:\codes\HumanActivityRecognition\data\downstream_data\df_with_windows_with_correct\df_ball_action_41.csv',
    signal_cols
)


df_ball_non_null = pd.read_csv(r'C:\codes\HumanActivityRecognition\data\downstream_data\df_ball_non_null.csv')
...
correct_files = [
    ...
    if f.startswith('df_ball_action_') and f.endswith('.csv')
]
dfs_correct = []
...
extra_cols = [
    ...
    # aggiungo qui tutte le colonne che vuoi portare da df_ball a df_ball_non_null
]
...
# Salvo il risultato
df_merged.to_csv(r'C:\codes\HumanActivityRecognition\data\downstream_data\df_ball_non_null_with_correctness.csv', index=False)



df_main = pd.read_csv(r'C:\codes\HumanActivityRecognition\data\downstream_data\3002_SP.csv')
df_correct = pd.read_csv(r'C:\codes\HumanActivityRecognition\data\downstream_data\df_ball_non_null_with_correctness.csv')
