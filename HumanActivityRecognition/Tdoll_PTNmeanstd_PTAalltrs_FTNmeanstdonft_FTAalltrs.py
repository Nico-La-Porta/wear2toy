...
path_1= 'C:\codes\HumanActivityRecognition\data\downstream_data\doll_3023_test'
path_2= 'C:\codes\HumanActivityRecognition\data\downstream_data\doll_3023_test_normalized_pt'
path_3= 'C:\codes\HumanActivityRecognition\data\downstream_data\doll_3023_test_normalized_ft'
logger.debug(path)
...
df_kid_test = df_doll_filtered[df_doll_filtered['kid_id'] == KID_TEST]

# Tieni i dati di tutti gli altri per training e tuning
df_doll_filtered = df_doll_filtered[df_doll_filtered['kid_id'] != KID_TEST]
...
logger.info(f"Righe training/val: {df_doll_filtered.shape[0]}")
...
means, stds = normalization.compute_dataframe_mean_std(df_doll_filtered)
logger.info(f"Media delle colonne: {means}")
logger.info(f"Deviazione standard delle colonne: {stds}")
df_doll_normalized = normalization.normalize_dataframe_mean_std(df_doll_filtered, means, stds)
...
for action_id in df_doll_normalized['action_id'].unique():
    df_action = df_doll_normalized[df_doll_normalized["action_id"] == action_id] #filtro il dataframe in base all'attività
    ...
    #salvo il dataframe
    df_action.to_csv(os.path.join(path_3,f'df_doll_action_{action_id}.csv'),index=False) #index=False per non salvare l'indice
...
for action_file in [f for f in os.listdir(path_3) if f.endswith('.csv') and f.split('_')[1] == 'doll']:
...
logger.info("=== INIZIO ELABORAZIONE TEST KID 3023 ===")
...
get_class_distribution(Y_remapped, "Doll Dataset con Etichette Rimappate")
get_class_distribution(Y_Test_remapped, "Doll Dataset Test con Etichette Rimappate")
...
doll_mapping = mapping_activity.DOLL_ACTION_MAPPING
print(f"Doll mapping keys: {list(doll_mapping.keys())}")

labels_dict = doll_mapping["encoded_to_name"]
print(f"Labels dict: {labels_dict}")

# Ottiengo il numero di classi
num_classes = len(np.unique(Y))
class_names = [doll_mapping["encoded_to_name"][i] for i in range(num_classes)]
...
OPTUNA_RESULTS_PATH = os.path.join(REPORTS_DIR, 'kfold_results_Tdoll_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs.csv')
OPTUNA_HYPERPARAMS_PATH = os.path.join(REPORTS_DIR, 'best_hyperparameters_kfold_Tdoll_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs.csv')
...
if OPTUNA_ALREADY_RUN:
    ...
    for fold, (train_val_idx, test_idx) in enumerate(skf.split(X, Y)):
        ...
        def objective_fold(trial):
            ...
            model.classification_head = nn.Linear(num_ftrs, 9)
            model.set_n_classes(9)
...
model_final.classification_head = nn.Linear(num_ftrs, 9)
model_final.set_n_classes(9)
...
FINAL_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_kfold_final_Tdoll_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs.pkl")
...
plot_CM(
    ...
    figure_name="cm_final_kfold_Tdoll_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs_all_data_trainval",
...
test_loss, test_acc, test_f1 = train_with_cm.evaluate_model(
    ...
    figure_name=f"cm_holdout_kid_{KID_TEST}_final_eval_Tdoll_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs",
