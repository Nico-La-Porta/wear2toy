...
for action_id in df_elep_filtered['action_id'].unique():
    ...
    df_action = pd.read_csv(os.path.join(path_1, f'df_elep_action_{action_id}.csv'))
    ...
    normalized_file_path = os.path.join(path_2, f'df_elep_action_{action_id}_normalized_pt.csv')
...
for action_file in [f for f in os.listdir(path_2) if f.endswith('.csv') and f.split('_')[1] == 'elep']:
...
OPTUNA_RESULTS_PATH = os.path.join(REPORTS_DIR, 'kfold_results_Telephant_PTNmeanstd_PTAalltrs_FTNmeanstdonpt_FTAalltrs.csv')
OPTUNA_HYPERPARAMS_PATH = os.path.join(REPORTS_DIR, 'best_hyperparameters_kfold_Telephant_PTNmeanstd_PTAalltrs_FTNmeanstdonpt_FTAalltrs.csv')
...
FINAL_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_kfold_final_Telephant_PTNmeanstd_PTAalltrs_FTNmeanstdonpt_FTAalltrs.pkl")
...
plot_CM(
    ...
    figure_name="cm_final_kfold_Telephant_PTNmeanstd_PTAalltrs_FTNmeanstdonpt_FTAalltrs_all_data_trainval",
...
test_loss, test_acc, test_f1 = train_with_cm.evaluate_model(
    ...
    figure_name=f"cm_holdout_kid_{KID_TEST_1}_{KID_TEST_2}_final_eval_Telephant_PTNmeanstd_PTAalltrs_FTNmeanstdonpt_FTAalltrs",
