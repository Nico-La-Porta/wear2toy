...
for i in range(X_Train.shape[0]):
    X_Train_normalized[i] = scaler.fit_transform(X_Train[i])  # Normalizza per finestra

for i in range(X_Test.shape[0]):
    X_Test_normalized[i] = scaler.transform(X_Test[i])  # Usa la stessa normalizzazione sui dati di test




# Creazione dataset
train_dataset = HARDataset(X_Train_normalized, Y_Train)
test_dataset = HARDataset(X_Test_normalized, Y_Test)
