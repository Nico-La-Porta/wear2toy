...
path_3= 'C:\codes\HumanActivityRecognition\data\downstream_data\car_3009_test_normalized_ft'
...
means, stds = normalization.compute_dataframe_mean_std(df_car_filtered)
logger.info(f"Media delle colonne: {means}")
logger.info(f"Deviazione standard delle colonne: {stds}")
df_car_normalized = normalization.normalize_dataframe_mean_std(df_car_filtered, means, stds)
...
for action_id in df_car_normalized['action_id'].unique():
    df_action = df_car_normalized[df_car_normalized["action_id"] == action_id] #filtro il dataframe in base all'attività
    ...
    #salvo il dataframe
    df_action.to_csv(os.path.join(path_3,f'df_car_action_{action_id}.csv'),index=False) #index=False per non salvare l'indice
...
for action_file in [f for f in os.listdir(path_3) if f.endswith('.csv') and f.split('_')[1] == 'car']:
...
OPTUNA_RESULTS_PATH = os.path.join(REPORTS_DIR, 'kfold_results_Tcar_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs.csv')
OPTUNA_HYPERPARAMS_PATH = os.path.join(REPORTS_DIR, 'best_hyperparameters_kfold_Tcar_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs.csv')
...
FINAL_MODEL_PATH = os.path.join(MODELS_DIR, "best_model_kfold_final_Tcar_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs.pkl")
...
plot_CM(
    ...
    figure_name="cm_final_kfold_Tcar_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs_all_data_trainval",
...
test_loss, test_acc, test_f1 = train_with_cm.evaluate_model(
    ...
    figure_name=f"cm_holdout_kid_{KID_TEST}_final_eval_Tcar_PTNmeanstd_PTAalltrs_FTNmeanstdonft_FTAalltrs",
    save_confusion_matrix=True,
    save_f1_score=True,
